...
app.get('/api/realprice', async (req, res) => {
    ...
    try {
        const { data } = await axios.get(`https://html.duckduckgo.com/html/?q=${encodeURIComponent(hotelName + ' 單人 一晚 住宿 價格 NT$')}`, {
            headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36' },
            timeout: 5000
        });
        const $ = cheerio.load(data);
        ...
        if (found) {
            ...
            return res.json({ minPrice, maxPrice, platform, roomType });
        } else {
            ...
            return res.json({ minPrice: simMinPrice, maxPrice: simMaxPrice, platform: simPlatform, roomType: '標準客房', isSimulated: true });
        }
    } catch (err) {
